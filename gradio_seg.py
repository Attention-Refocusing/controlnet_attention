...
def generate_seg_paper(detected_map):
    ...
    cv2.imwrite('paper/dog_horse.png',seg_img)
...
def get_segment(det, input_image):
    ...
    cv2.imwrite('seg//mask1.png',mask_1)
    cv2.imwrite('seg/dog_teddy/mask2.png',mask_2)
...
if __name__=='__main__':
    ...
    for n in img_names:
        print (n)
        # input_image = cv2.imread(os.path.join('try_img/img_7', n))
        # import pdb; pdb.set_trace()
        # n = 
        # input_image = cv2.imread(os.path.join('try_img/img4', n))
        # input_image = cv2.imread('/vulcanscratch/quynhpt/ControlNet-v1-1-nightly/img_depth/baby/baby_harder.jpeg')
        input_image = cv2.imread('/vulcanscratch/quynhpt/ControlNet-v1-1-nightly/try_img/img_sheep/671101b2ea96da21a0a25fe1fcc9b854.jpeg')
        # input_image = cv2.imread('/vulcanscratch/quynhpt/ControlNet-v1-1-nightly/try_img/img_2/girl_dpg.png')
        prompt = 'a dog and a sheep'
        # prompt = 'a baby and a dog sitting on the carpet'
        # prompt = 'a dog and a teddy bear sitting in the bed room'
        ...
        for i in range(0, 16):
            # if (i!=15 and i!=0 and i!=1): continue
            seed = i
            ...
            cv2.imwrite('seg/sheep/loss'+ n,results[0])
            cv2.imwrite('seg/sheep/loss' +n  + str(i)+'_.png',results[1][:,:,::-1])
        break
    # import pdb; pdb.set_trace()
